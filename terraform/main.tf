...
resource "aws_s3_bucket" "shopping_list_backup" {
...
resource "aws_s3_bucket_versioning" "shopping_list_backup" {
  bucket = aws_s3_bucket.shopping_list_backup.id
...
resource "aws_s3_bucket_server_side_encryption_configuration" "shopping_list_backup" {
  bucket = aws_s3_bucket.shopping_list_backup.id
...
resource "aws_s3_bucket_public_access_block" "shopping_list_backup" {
  bucket = aws_s3_bucket.shopping_list_backup.id
    ...
    Statement = [
      {
        ...
        Resource = [
          aws_s3_bucket.shopping_list_backup.arn,
          "${aws_s3_bucket.shopping_list_backup.arn}/*"
        ]
      }
    ]
...
# Custom EBS CSI driver policy for EKS node group role
resource "aws_iam_policy" "node_ebs_csi" {
  name        = "${var.project_name}-node-ebs-csi-policy"
...
# Attach EBS CSI driver policy to EKS node group role
...
resource "aws_iam_role" "ebs_csi_controller" {
  ...
  name               = "${var.project_name}-ebs-csi-controller"
}

# EBS CSI Controller Policy
resource "aws_iam_policy" "ebs_csi_controller" {
  name        = "${var.project_name}-ebs-csi-controller-policy"
...
output "s3_bucket_name" {
  ...
  value       = aws_s3_bucket.shopping_list_backup.bucket
}
